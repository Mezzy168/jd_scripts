...
if ($.cointowallet.returncode==0){ $.message +=`【现金提现】：成功提现${CASH}元,执行时间${$.cointowallet.result.cashtime}\n`}
else{ $.message +=`【现金提现】：${$.cointowallet.message},执行时间${$.cointowallet.result.cashtime}\n`}
        } catch (e) {
          $.logErr(e, resp);
        } finally {
          resolve()
